...
| GET | /api/user/orders | 用户中心"我的订单"(兼容单体遗留入口,实现同 /api/orders) |
...
| ④ | user-service | order-service | GET /internal/admin/stats/orders/summary、/orders/trends | 后台看板:订单总量/总销售额/近7天趋势 |
| ⑤ | user-service | product-service | GET /internal/product/hot | 后台看板:销量 Top10 产品 |
...
| /api/flights/order、/api/trains/order、/api/vacations/order、/api/hotel/order、/api/user/orders、/api/orders、/api/admin/orders | order-service |
